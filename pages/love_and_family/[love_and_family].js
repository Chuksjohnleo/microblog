...
export async function getStaticProps({params}){
    ...
    try {
      ...
      let body = await category.findOne({id:params.love_and_family},{projection:{_id:0}});
      let comments = await commentCollection.find({postId: params.love_and_family},{projection:{_id:0}}).sort({_id: -1}).toArray()
...
export async function getStaticPaths() {
   ...
   const paths =  pathsArr.map((path)=>{
        return {
          params:{love_and_family:  path.id.toString()}
        }
    });
